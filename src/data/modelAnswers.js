export const modelAnswers = {
  ...
  'intro-2': `저는 두 가지 측면에서 SK하이닉스 AE에 적합하다고 생각합니다. 첫째, AI 가속기 회사에서 실제 AI 워크로드를 다뤄본 경험입니다. LLM 추론 과정에서 메모리 대역폭이 전체 성능의 병목이 되는 구간을 직접 분석했기 때문에, 고객이 어떤 맥락에서 메모리 성능을 요구하는지 체감적으로 이해하고 있습니다. 둘째, RTL 설계부터 C++ 모델링, 시스템 시뮬레이션까지 다층적 분석 역량을 갖추고 있습니다. 게이트 레벨의 문제가 시스템 레벨에서 어떻게 드러나는지 연결 지어 볼 수 있습니다. 입사 후에는 고객 시스템에서의 최적 메모리 동작 조건을 도출하고, cHBM 시대에 고객별 아키텍처를 이해한 기술 컨설턴트로 성장하는 것이 목표입니다.`,
  ...
  'intro-5': `SK하이닉스 AE에 지원한 이유는 크게 두 가지입니다. 먼저 AE라는 직무 자체에 매력을 느꼈습니다. PE가 단품의 안정화에 집중한다면, AE는 고객의 시스템 레벨에서 메모리를 검증하고 인증하는 역할입니다. 저는 시스템 전체 맥락 속에서 문제를 찾고 소통하는 일이 제 역량과 잘 맞는다고 생각했습니다. 다음으로, SK하이닉스가 Creator로의 전환을 선언하면서 AE의 역할이 단순 기술지원을 넘어 기술 컨설팅으로 확장되고 있다는 점에 큰 가능성을 느꼈습니다. 저를 한마디로 표현하면 "기술 사이의 가능성을 찾는 엔지니어"입니다. SW와 HW의 경계, 설계와 시스템의 경계에서 연결점을 찾아온 경험이 AE로서의 강점이 될 것입니다.`,
